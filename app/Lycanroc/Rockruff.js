import PokemonTemplate from '../../components/PokemonTemplate';

export default function Charizard() {
  const data = {
    name: 'Rockruff',
    type1: 'Rock',
    type2: null,
    shinyImage: { uri: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTtDtHR7EoRWRDuNVhprkSg1qq3S1oQ2GuiWQ&s' },
    evolutionMethod: 'base pokemon form',
    funFact: 'Rockruff has the ability to use the scent of its trainer to track them for days if it gets lost',
    evolutions: [
      { name: 'Rockruff', link: '/Lycanroc/Rockruff', image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTtDtHR7EoRWRDuNVhprkSg1qq3S1oQ2GuiWQ&s', type: 'Rock' },
      { name: 'Lycanroc', link: '/Lycanroc/Lycanroc', image: 'https://images-wixmp-ed30a86b8c4ca887773594c2.wixmp.com/f/a25bad2e-b103-45c6-96b5-7a2e2e2b709d/dbxriou-95345834-1695-4f8f-b1b9-52faaf247f51.png?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..nZEv4PQ9Cju0YAwHDpTmxvwi2Qz2aBRBnLTTtMnAfKg', type: 'Rock' },]
  };
  return <PokemonTemplate route={{ params: data }} />;
}